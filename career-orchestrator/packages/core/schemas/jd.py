...
class JDKeyword(BaseModel):
    ...
    text: str = Field(..., min_length=1, description="키워드 원문 표기(예: 'CI/CD', 'K8s')")
    category: JDCategory = Field(default="other")
    # JD 내 근거 문장(짧게). missing validation/설명에 유용(선택)
    evidence: Optional[str] = Field(default=None, description="JD에서 해당 키워드가 등장한 근거 문장(선택)")
    # 중요도(선택): 나중에 required 내부에서도 가중치 세분화 가능
    importance: Optional[int] = Field(default=None, ge=1, le=5, description="1(낮음)~5(높음), 선택")
...
class JDSection(BaseModel):
    ...
    keywords: list[str] = Field(default_factory=list, description="이 섹션에서 추출된 키워드(선택)")
...
class JDProfile(BaseModel):
    ...
    raw_text: Optional[str] = Field(default=None, description="사용자가 붙여넣은 원문 JD(선택)")
    role_title: Optional[str] = Field(default=None, description="직무 타이틀(추출 가능하면)")
    ...
    def normalize_sections(cls, v: list[JDSection]) -> list[JDSection]:
        for s in v:
            ...
            if s.keywords:
                s.keywords = _dedupe_case_insensitive([_norm_text(k) for k in s.keywords if k.strip()])
        return v
    ...
    def normalize_keywords(cls, v: list[JDKeyword]) -> list[JDKeyword]:
        for k in v:
            k.text = _norm_text(k.text)
            if k.evidence:
                k.evidence = _norm_text(k.evidence)
        return v
